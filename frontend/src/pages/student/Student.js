...
import { ADD_INFO_COURSE_DRAWER, SET_EDIT_STUDENT } from "../../redux/types";
import {
  getStudentsActionApi,
} from "../../actions/api-actions/ProgramAction";
import FormStudent from "../../components/form-student/FormStudent";
...
export default function Student() {
  ...
  const sortList = ['id', 'name', 'major', 'year', 'email']
  const [searchedObj, setSearchedObj] = useState([])
  const [page, setPage] = useState(1);
  const { students } = useSelector((state) => state.GeneralProgramReducer);
  ...
  const handleSearch = () => {
    dispatch(getStudentsActionApi(page, searchedObj));
  }
  ...
  const handleReset = (dataIndex, clearFilters) => {
    ...
    dispatch(getStudentsActionApi(page, searchedObjUpdate));
  }
  ...
  const data = students?.data?.map((genPro, idx) => {
    return {
      ...genPro,
      key: (page - 1) * 10 + idx + 1,
    };
  ...
  return (
    <div className="mt-2 general-program">
      ...
      <div className="d-flex justify-content-center my-3">
        <Pagination current={page} total={students.total} onChange={onChange} />
      </div>
    </div>
  );
}

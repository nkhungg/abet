...
import { Table, Input, Pagination, Menu, Dropdown, Button } from "antd";
import { FormattedMessage } from "react-intl";
import { NavLink } from "react-router-dom";
import { SearchOutlined } from "@ant-design/icons";
import { Switch } from "antd";
import { editVersionSurveyActionApi, getSurveysActionApi } from "../../actions/api-actions/ProgramAction";
import { ADD_INFO_COURSE_DRAWER, SET_EDIT_SURVEY, SET_PROGRAM_VERSION_INFO_FROM_SURVEY_OUTSIDE } from "../../redux/types";
import FormSurvey from "../../components/form-survey/FormSurvey";
...
export default function Survey() {
  ...
  const { surveys, selectedFlowProgramVersion } = useSelector(
  ...
  const onChange = async (page) => {
    setPage(page);
    dispatch(getSurveysActionApi(page, searchedObj));
  };
  ...
  const handleReset = (dataIndex, clearFilters) => {
    ...
    dispatch(getSurveysActionApi(page, searchedObjUpdate));
  }
    ...
    },
    {
      ...
      render: (record) => (
        <span className="d-flex justify-content-center align-items-center">
          {record.lock && (
            <span
              // onClick={() => dispatch(deleteVersionSurveyActionApi({...record, programVersionInfo}))}
  ...
  const data = surveys?.data?.map((outcomeParent, idx) => {

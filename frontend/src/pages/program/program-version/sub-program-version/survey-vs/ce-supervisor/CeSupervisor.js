...
import CeSupervisorStat from './CeSupervisorStat';
import { MemoizedTemplateCEDSupervisor } from "../../../../../../components/modal-template/ModalTemplate";
...
const ButtonImport = () => {
    ...
    const saveImportFile = () => {
        ...
        let data = { programVersionInfo, surveyName, surveyType: 'ce-supervisor', fileInfo }
        dispatch(postImportFileSupervisorResultActionApi(data))
        setShowModal(false)
        setFile(null)
        document.getElementById("file-ce-supervisor").value = ''
    }
    ...
    return <div className="mr-4">
        ...
        >
            <MemoizedTemplateCEDSupervisor />
            <div className="text-center mb-4">
                <input
                    id="file-ce-supervisor"
                    onChange={changeFile}
                    type='file'
                    accept=".xls,.xlsx"
                />
...
export default function CeSupervisor() {
    ...
    useEffect(() => {
        if (programVersionInfo && surveyName) {
            dispatch(getVersionSurveyResultActionApi(programVersionInfo, surveyName, 'ce-supervisor', 1));
        }
    ...
    const onChange = (page) => {
        setPage(page)
        dispatch(getVersionSurveyResultActionApi(programVersionInfo, surveyName, 'ce-supervisor', page));
    };
    ...
    const postMatrix = () => {
        let dataUpdate = { selectedCellArray: [...selectedCellArray], programVersionInfo, surveyName, surveyType: 'ce-supervisor' }
    ...
    const confirm = (lecturerIdVal, studentIdVal) => {
        let dataUpdate = { selectedRow: { lecturerId: lecturerIdVal, studentId: studentIdVal }, programVersionInfo, surveyName, surveyType: 'ce-supervisor' }
        dispatch(deleteVersionSurveyResultActionApi(dataUpdate))
    }

    const changeSelectCell = (e, eleCell) => {
        let dataCellUpdate = { ...dataCell }
        let row = dataCellUpdate?.rowList?.find(item => item.title === eleCell.lecturerId && item.subTitle === eleCell.studentId)
    ...
    return (
        <div className="matrix">
            <div className="d-flex justify-content-between align-items-center mb-2">
                <div>
                    <h5 className="">
                        <FormattedMessage id="RESULT_CE_SUPERVISOR" />
                    </h5>
                    ...
                    >
                        ...
                        <tbody id="foundation-test-result">
                            {dataCell?.rowList.map((row, rowIdx) => {
                                row.lecturerId = (((row || {}).data || [])[0] || {}).lecturerId
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {isEdit && <Td style={{ padding: '0 20px', color: '#707070', cursor: 'pointer' }}><Popconfirm placement="topLeft" title={text} onConfirm={() => confirm(row.lecturerId, row.studentId)} okText="Yes" cancelText="No"><span className='fa fa-trash-alt'></span></Popconfirm></Td>}
                                    </tr>
                                );
                            })}
                        </tbody>

...
const ButtonImport = () => {
    ...
    const saveImportFile = () => {
        ...
        let data = { programVersionInfo, surveyName, surveyType: 'member', fileInfo }
        dispatch(postImportFileSupervisorResultActionApi(data))
        setShowModal(false)
        setFile(null)
        document.getElementById("file-member").value = '';
    }
...
export default function Member() {
    ...
    useEffect(() => {
        if (programVersionInfo && surveyName) {
            dispatch(getVersionSurveyResultActionApi(programVersionInfo, surveyName, 'member', 1));
        }
    ...
    const onChange = (page) => {
        setPage(page)
        if (programVersionInfo, surveyName) {
            dispatch(getVersionSurveyResultActionApi(programVersionInfo, surveyName, 'member', page));
        }
    };
    ...
    const postMatrix = () => {
        let dataUpdate = { selectedCellArray: [...selectedCellArray], programVersionInfo, surveyName, surveyType: 'member' }
    ...
    const confirm = (value) => {
        let dataUpdate = { selectedRow: { doProjectId: value }, programVersionInfo, surveyName, surveyType: 'member' }

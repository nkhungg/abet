...
import { MemoizedTemplateCommitte } from "../../../../../../components/modal-template/ModalTemplate";
...
const ButtonImport = () => {
    ...
    const saveImportFile = () => {
        ...
        let data = { programVersionInfo, surveyName, surveyType: 'committee', fileInfo }
        dispatch(postImportFileSupervisorResultActionApi(data))
        setShowModal(false)
        setFile(null)
        document.getElementById("file-committee").value = '';
    }
...
export default function Committee() {
    ...
    useEffect(() => {
        if (programVersionInfo && surveyName) {
            dispatch(getVersionSurveyResultActionApi(programVersionInfo, surveyName, 'committee', 1));
        }
    ...
    const onChange = (page) => {
        setPage(page)
        dispatch(getVersionSurveyResultActionApi(programVersionInfo, surveyName, 'committee', page));
    };
    ...
    const postMatrix = () => {
        let dataUpdate = { selectedCellArray: [...selectedCellArray], programVersionInfo, surveyName, surveyType: 'committee' }
    ...
    const confirm = (lecturerIdVal, studentIdVal) => {
        let dataUpdate = { selectedRow: { lecturerId: lecturerIdVal, studentId: studentIdVal }, programVersionInfo, surveyName, surveyType: 'committee' }
        dispatch(deleteVersionSurveyResultActionApi(dataUpdate))
    }

    const changeSelectCell = (e, eleCell) => {
        let dataCellUpdate = { ...dataCell };
        let row = dataCellUpdate?.rowList?.find(item => item.subTitle === eleCell.studentId);
    ...
    return (
        <div className="matrix">
            <div className="d-flex justify-content-between align-items-center mb-2">
                <div>
                    <h5 className="">
                        <FormattedMessage id="RESULT_COMMITTEE" />
                    </h5>

...
import CeHdStat from "./CeHdStat";
import ModalImport from "../../../../../../components/modal-import/ModalImport";
import { MemoizedTemplateCEHD } from "../../../../../../components/modal-template/ModalTemplate";
...
const ButtonImport = () => {
    ...
    const saveImportFile = () => {
        ...
        let data = { programVersionInfo, surveyName, surveyType: 'ce-hd', fileInfo }
        dispatch(postImportFileSupervisorResultActionApi(data))
        setShowModal(false)
        setFile(null)
        document.getElementById("file-ce-hd").value = '';
    }
...
export default function CeHd() {
    ...
    useEffect(() => {
        if (programVersionInfo && surveyName) {
            dispatch(getVersionSurveyResultActionApi(programVersionInfo, surveyName, 'ce-hd', 1));
        }
    ...
    const onChange = (page) => {
        setPage(page)
        dispatch(getVersionSurveyResultActionApi(programVersionInfo, surveyName, 'ce-hd', page));
    };
    ...
    const postMatrix = () => {
        let dataUpdate = { selectedCellArray: [...selectedCellArray], programVersionInfo, surveyName, surveyType: 'ce-hd' }
    ...
    const confirm = (lecturerIdVal, studentIdVal) => {
        let dataUpdate = { selectedRow: { lecturerId: lecturerIdVal, studentId: studentIdVal }, programVersionInfo, surveyName, surveyType: 'ce-hd' }
    ...
    return (
        <div className="matrix">
            <div className="d-flex justify-content-between align-items-center mb-2">
                <div>
                    <h5 className="">
                        <FormattedMessage id="RESULT_CE_HD" />
                    </h5>

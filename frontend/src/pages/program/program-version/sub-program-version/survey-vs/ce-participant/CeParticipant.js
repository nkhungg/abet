...
import CeParticipantStat from './CeParticipantStat'
import { MemoizedTemplateCEDParticipant } from "../../../../../../components/modal-template/ModalTemplate";
...
const ButtonImport = () => {
    ...
    const saveImportFile = () => {
        ...
        let data = { programVersionInfo, surveyName, surveyType: 'ce-participant', fileInfo }
    ...
    return <div className="mr-4">
        ...
        >
            <MemoizedTemplateCEDParticipant />
...
export default function CeParticipant() {
    ...
    useEffect(() => {
        if (programVersionInfo && surveyName) {
            dispatch(getVersionSurveyResultActionApi(programVersionInfo, surveyName, 'ce-participant', 1));
        }
    ...
    const onChange = (page) => {
        setPage(page)
        dispatch(getVersionSurveyResultActionApi(programVersionInfo, surveyName, 'ce-participant', page));
    };
    ...
    const postMatrix = () => {
        let dataUpdate = { selectedCellArray: [...selectedCellArray], programVersionInfo, surveyName, surveyType: 'ce-participant' }
    ...
    const confirm = (participantVal, studentIdVal) => {
        let dataUpdate = { selectedRow: { participant: participantVal, studentId: studentIdVal }, programVersionInfo, surveyName, surveyType: 'ce-participant' }
        dispatch(deleteVersionSurveyResultActionApi(dataUpdate))
    }

    const changeSelectCell = (e, eleCell) => {
        let dataCellUpdate = { ...dataCell }
        let row = dataCellUpdate?.rowList?.find(item => item.title === eleCell.participant && item.subTitle === eleCell.studentId)
    ...
    return (
        <div className="matrix">
            <div className="d-flex justify-content-between align-items-center mb-2">
                <div>
                    <h5 className="">
                        <FormattedMessage id="RESULT_CE_PARTICIPANT" />
                    </h5>
                    ...
                    >
                        ...
                        <tbody id="foundation-test-result">
                            {dataCell?.rowList.map((row, rowIdx) => {
                                row.participant = (((row || {}).data || [])[0] || {}).participant
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {isEdit && <Td style={{ padding: '0 20px', color: '#707070', cursor: 'pointer' }}><Popconfirm placement="topLeft" title={text} onConfirm={() => confirm(row.participant, row.studentId)} okText="Yes" cancelText="No"><span className='fa fa-trash-alt'></span></Popconfirm></Td>}
                                    </tr>
                                );
                            })}
                        </tbody>

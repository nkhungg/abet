...
import { MemoizedTemplateReviewer } from "../../../../../../components/modal-template/ModalTemplate";
...
const ButtonImport = () => {
    ...
    const saveImportFile = () => {
        ...
        let data = { programVersionInfo, surveyName, surveyType: 'reviewer', fileInfo }
        dispatch(postImportFileSupervisorResultActionApi(data))
        setShowModal(false)
        setFile(null)
        document.getElementById("file-reviewer").value = '';
    }
...
export default function Reviewer() {
    ...
    useEffect(() => {
        if (programVersionInfo && surveyName) {
            dispatch(getVersionSurveyResultActionApi(programVersionInfo, surveyName, 'reviewer', 1));
        }
    ...
    const onChange = (page) => {
        setPage(page)
        dispatch(getVersionSurveyResultActionApi(programVersionInfo, surveyName, 'reviewer', page));
    };
    ...
    const postMatrix = () => {
        let dataUpdate = { selectedCellArray: [...selectedCellArray], programVersionInfo, surveyName, surveyType: 'reviewer' }
    ...
    const confirm = (value) => {
        let dataUpdate = { selectedRow: { doProjectId: value }, programVersionInfo, surveyName, surveyType: 'reviewer' }
    ...
    return (
        <div className="matrix">
            <div className="d-flex justify-content-between align-items-center mb-2">
                <div>
                    <h5 className="">
                        <FormattedMessage id="RESULT_REVIEWER" />
                    </h5>
                    ...
                    >
                        ...
                        <tbody id="foundation-test-result">
                            {dataCell?.rowList.map((row, rowIdx) => {
                                row.doProjectId = (((row || {}).data || [])[0] || {}).doProjectId
                                return (
                                    <tr
                                    ...
                                    >
                                        ...
                                        {isEdit && <Td style={{ padding: '0 20px', color: '#707070', cursor: 'pointer' }}><Popconfirm placement="topLeft" title={text} onConfirm={() => confirm(row.doProjectId)} okText="Yes" cancelText="No"><span className='fa fa-trash-alt'></span></Popconfirm></Td>}
                                    </tr>
                                );
                            })}
                        </tbody>

...
import InternshipStat from './InternshipStat'
import { MemoizedTemplateIntern } from "../../../../../../components/modal-template/ModalTemplate";
...
const ButtonImport = () => {
    ...
    const saveImportFile = () => {
        ...
        let data = { programVersionInfo, surveyName, surveyType: 'internship', fileInfo }
        dispatch(postImportFileSupervisorResultActionApi(data))
        setShowModal(false)
        setFile(null)
        document.getElementById("file-internship").value = '';
    }
...
export default function Internship() {
    ...
    useEffect(() => {
        if (programVersionInfo, surveyName) {
            dispatch(getVersionSurveyResultActionApi(programVersionInfo, surveyName, 'internship', 1));
        }
    ...
    const onChange = (page) => {
        setPage(page)
        dispatch(getVersionSurveyResultActionApi(programVersionInfo, surveyName, 'internship', page));
    };
    ...
    const postMatrix = () => {
        let dataUpdate = { selectedCellArray: [...selectedCellArray], programVersionInfo, surveyName, surveyType: 'internship' }
    ...
    const confirm = (value) => {
        let dataUpdate = { selectedRow: { internId: value }, programVersionInfo, surveyName, surveyType: 'internship' }
    ...
    return (
        <div className="matrix">
            <div className="d-flex justify-content-between align-items-center mb-2">
                <div>
                    <h5 className="">
                        <FormattedMessage id="RESULT_INTERNSHIP" />
                    </h5>
                    ...
                    >
                        ...
                        <tbody id="foundation-test-result">
                            {dataCell?.rowList.map((row, rowIdx) => {
                                row.internId = (((row || {}).data || [])[0] || {}).internId
                                return (
                                    <tr
                                    ...
                                    >
                                        ...
                                        {isEdit && <Td style={{ padding: '0 20px', color: '#707070', cursor: 'pointer' }}><Popconfirm placement="topLeft" title={text} onConfirm={() => confirm(row.internId)} okText="Yes" cancelText="No"><span className='fa fa-trash-alt'></span></Popconfirm></Td>}
                                    </tr>
                                );
                            })}
                        </tbody>

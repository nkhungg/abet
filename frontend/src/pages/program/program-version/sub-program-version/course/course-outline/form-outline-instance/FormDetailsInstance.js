...
import { deleteOutlineDetailFromCourseInstanceActionApi, editOutlineDetailFromCourseInstanceActionApi, getParentOutcomeAutocompleteInCourseInstanceActionApi, postOutlineDetailFromCourseInstanceActionApi } from "../../../../../../../actions/api-actions/ProgramAction";
const { Option } = Select;

const textConfirm = "Are you sure to delete this task?";
function FormDetailsInstance(props) {
    // editOutlineDetailFromCourseInstance
    const dispatch = useDispatch();
  const { values, handleChange, handleSubmit, setFieldValue } = props;
  const { selectedFlowProgramVersion, parentIdAutocompleteCourseInstance } = useSelector((state) => state.GeneralProgramReducer);
  ...
  const confirmDelete = (value) => {
    let valueUpdate = {...value, programVersionInfo}
    dispatch(deleteOutlineDetailFromCourseInstanceActionApi(valueUpdate))
  };
  ...
  const renderLecturers = () => {
    return parentIdAutocompleteCourseInstance?.data?.map((item, idx) => {
    ...
    return (
        ...
        <form className="form-create-course" onSubmit={handleSubmit}>
          ...
          <div className="col-12 mt-2 d-flex justify-content-center">
            { props.editOutlineDetailFromCourseInstance.isEdit &&
              <Popconfirm
              placement="topLeft"
              title={textConfirm}
...
const FormDetailsInstanceFormik = withFormik({
    enableReinitialize: true,
    mapPropsToValues: (props) => {
      const { editOutlineDetailFromCourseInstance } = props;
      return {
        programId: editOutlineDetailFromCourseInstance?.programId,
        courseId: editOutlineDetailFromCourseInstance?.courseId,
        id: editOutlineDetailFromCourseInstance?.id,
        chapter: editOutlineDetailFromCourseInstance?.chapter,
        content: editOutlineDetailFromCourseInstance?.content,
        outputStandard: editOutlineDetailFromCourseInstance?.outputStandard,
        reviewAction: editOutlineDetailFromCourseInstance?.reviewAction,
        type: editOutlineDetailFromCourseInstance?.type,
        outcomeIdList: [],
      };
    },
    validationSchema: Yup.object().shape({}),
    handleSubmit: (values, { props, setSubmitting }) => {
      const { editOutlineDetailFromCourseInstance, selectedFlowProgramVersion } = props;
      ...
      if(editOutlineDetailFromCourseInstance.isEdit) {
        props.dispatch(editOutlineDetailFromCourseInstanceActionApi(valueUpdate))
      } else {
        props.dispatch(postOutlineDetailFromCourseInstanceActionApi(valueUpdate))
      }
    },
  })(FormDetailsInstance);
  ...
  const mapStateToProps = (state) => ({
    editOutlineDetailFromCourseInstance:
      state.GeneralProgramReducer.editOutlineDetailFromCourseInstance,
  });
  
  export default connect(mapStateToProps)(FormDetailsInstanceFormik);
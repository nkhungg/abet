...
import FormDetails from "./form-outline/FormDetails";
import {
  ADD_INFO_COURSE_DRAWER,
  SET_DETAILS_IN_COURSE_OUTLINE,
} from "../../../../../../../redux/types";
import { useDispatch, useSelector } from "react-redux";
import { getCourseDetailInCourseOutlineActionApi } from "../../../../../../../actions/api-actions/ProgramAction";
import ReactHtmlParser from "react-html-parser";
import "./CourseOutline.scss";

export default function OutlineDetails() {
  const dispatch = useDispatch();
  const { selectedFolw, detailsFromCourseOutline } = useSelector(
    (state) => state.GeneralProgramReducer
  );
  useEffect(() => {
    dispatch(
      getCourseDetailInCourseOutlineActionApi(
  ...
  const data = detailsFromCourseOutline?.data?.map((courseDetail) => {
        ...
        <span className="d-flex justify-content-center align-items-center">
          <span
            onClick={() => {
              ...
              const action2 = {
                type: SET_DETAILS_IN_COURSE_OUTLINE,

...
import { ADD_INFO_COURSE_DRAWER, SET_EDIT_USER } from "../../redux/types";
import {
  getUsersActionApi,
...
export default function Users() {
  ...
  const { users } = useSelector((state) => state.GeneralProgramReducer);
  ...
  const handleSearch = () => {
    dispatch(getUsersActionApi(page, searchedObj));
  }
  ...
  const handleReset = (dataIndex, clearFilters) => {
    ...
    dispatch(getUsersActionApi(page, searchedObjUpdate));
  }
  ...
  const data = users?.data?.map((genPro, idx) => {
    return {
      ...genPro,
      key: (page - 1) * 10 + idx + 1,
    };
  ...
  return (
    <div className="mt-2 general-program">
      ...
      <div className="d-flex justify-content-center my-3">
        <Pagination current={page} total={users.total} onChange={onChange} />
      </div>
    </div>
  )
}
